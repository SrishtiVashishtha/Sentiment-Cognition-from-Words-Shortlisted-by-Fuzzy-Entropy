...
print("\n Global Shortlisted words in testing: "+ str(global_shortlisted))

TEST_GS_POS=global_shortlisted
...
print("\n Global Shortlisted words in testing: "+ str(global_shortlisted2))

TEST_GS_NEG=global_shortlisted2
...
Test_SH_P_W=rearrange_whole(TEST_GS_POS)
Test_SH_N_W=rearrange_whole(TEST_GS_NEG)
...
SH_POS_Test=calc_wordtonum_para(Test_SH_P_W)
SH_NEG_Test=calc_wordtonum_para(Test_SH_N_W)

np_pos_test=np.ones((len(SH_POS_Test),), dtype=int)
np_neg_test=np.zeros((len(SH_NEG_Test),), dtype=int)

Test=[]  #Contains all the testing data of positive and negative reviews
for i in range(len(SH_POS_Test)):
    Test.append(SH_POS_Test[i])
for j in range(len(SH_NEG_Test)):
    Test.append(SH_NEG_Test[j])
#print(Test)
    
Test_labels=[] #Contains the sentiment labels for all testing data of positive and negative reviews
for i in range(len(np_pos_test)):
    Test_labels.append(np_pos_test[i])
for j in range(len(np_neg_test)):
    Test_labels.append(np_neg_test[j])
#print(Test_labels)
...
file_name = "testfile.npz".format(i)
final_name=os.path.join(BASE_PATH, file_name)
np.savez(final_name,Testset=Test,TestLabels=Test_labels)
